...
func TestSimpleJson(t *testing.T) {
	...
	if _, err := Generate(i, ParseJson, "TestStruct", []string{"json"}, false, true); err != nil {
...
func TestInferFloatInt(t *testing.T) {
	...
	actual, err := Generate(f, ParseJson, "Stats", []string{"json"}, false, true)
	if err != nil {
		t.Error(err)
	}
	sactual, sexpected := strings.TrimSpace(string(actual)), strings.TrimSpace(string(expected))
...
func TestYamlNumbers(t *testing.T) {
	...
	actual, err := Generate(f, ParseYaml, "Stats", []string{"yaml"}, false, false)
	if err != nil {
		t.Error(err)
	}
	sactual, sexpected := strings.TrimSpace(string(actual)), strings.TrimSpace(string(expected))
...
func TestExample(t *testing.T) {
	...
	actual, err := Generate(i, ParseJson, "User", []string{"json"}, false, true)
	if err != nil {
		t.Error(err)
	}
	sactual, sexpected := strings.TrimSpace(string(actual)), strings.TrimSpace(string(expected))
